...
The search script lives inside this skill's own directory, not the project directory. Always invoke it by its full path — do not assume a particular working directory. Replace `{skill_dir}` with the absolute directory containing this SKILL.md. The examples use `uv run`; if uv is unavailable, use the project's existing Python interpreter:

```bash
uv run python "{skill_dir}/scripts/search.py" "<query>" --domain <domain>
```

Requires Python 3.x and no external dependencies. If no Python interpreter is available, ask the user before installing one; do not modify the system environment automatically.
...
```bash
uv run python "{skill_dir}/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
uv run python "{skill_dir}/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
uv run python "{skill_dir}/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
uv run python "{skill_dir}/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
uv run python "{skill_dir}/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
uv run python "{skill_dir}/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
uv run python "{skill_dir}/scripts/search.py" "<keyword>" --stack <stack>
```
...
uv run python "{skill_dir}/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
uv run python "{skill_dir}/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
uv run python "{skill_dir}/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
