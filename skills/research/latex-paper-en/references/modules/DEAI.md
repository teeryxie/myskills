...
```python
uv run --python 3.10 python scripts/deai_check.py main.tex --section introduction
# Output: AI trace analysis + rule-based revision suggestions
```

**Batch processing** (for entire chapters):
```bash
uv run --python 3.10 python scripts/deai_batch.py main.tex --chapter chapter3/introduction.tex
uv run --python 3.10 python scripts/deai_batch.py main.tex --all-sections  # Process entire document
```
...
```bash
uv run --python 3.10 python scripts/deai_check.py main.tex --analyze
