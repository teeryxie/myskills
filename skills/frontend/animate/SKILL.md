...
**Step 1 — Decision Ladder**: run the Decision Ladder from `../ui-craft/references/motion.md` first. Anything that fails it gets removed, not improved.

**Step 2 — Pick the library**:

- If user opted into a stack during Discovery → read `../ui-craft/references/stack.md`. Use the matching section (Motion, GSAP, or Three.js). Never mix libraries on the same property.
...
Exit ≈ 75% of entrance duration (shorter, same `ease-out` — or a flatter tail like `cubic-bezier(0.4, 0, 1, 1)` for a softer exit). Never `ease-in` on UI (see `../ui-craft/references/motion.md`). `cubic-bezier(0.22, 1, 0.36, 1)` is a safe spring-like default.
