...
```bash
uv run --python 3.10 python scripts/optimize_title.py main.tex --generate
# Analyzes abstract/introduction to propose 3-5 title candidates
```

**Optimize existing title**:
```bash
uv run --python 3.10 python scripts/optimize_title.py main.tex --optimize
# Analyzes current title and provides improvement suggestions
```

**Check title quality**:
```bash
uv run --python 3.10 python scripts/optimize_title.py main.tex --check
...
```bash
uv run --python 3.10 python scripts/optimize_title.py main.tex --interactive
# Step-by-step guided title creation with user input
```

**Batch Mode** (For multiple papers):
```bash
uv run --python 3.10 python scripts/optimize_title.py "papers/*.tex" --batch --output title_report.json
```

**Title A/B Testing** (Optional):
```bash
uv run --python 3.10 python scripts/optimize_title.py main.tex --compare "Title A" "Title B" "Title C"
