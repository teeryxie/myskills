...
allowed-tools: Read, Glob, Grep, Bash(uv run *), Bash(pdflatex *), Bash(xelatex *), Bash(latexmk *), Bash(bibtex *), Bash(biber *), Bash(chktex *)
---
...
4. Run the required script (e.g., `uv run --python 3.10 python $SKILL_DIR/scripts/check_format.py main.tex`).
...
- Run `uv run --python 3.10 python $SKILL_DIR/scripts/check_format.py $ARGUMENTS`.
- Run `uv run --python 3.10 python $SKILL_DIR/scripts/compile.py $ARGUMENTS`.
