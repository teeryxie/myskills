...
Design every non-happy state for the UI at `$ARGUMENTS`. Load the `ui-craft` skill and read `../ui-craft/references/state-design.md`.
...
**Step 2 — Fill the missing states.** For each missing state, either stub it inline or add a follow-up task comment. Use `../ui-craft/references/state-design.md` for:
