...
Redesign `$ARGUMENTS`. Load the `ui-craft` skill and read `../ui-craft/references/craft-intent.md`. A redesign is not a greenfield build wearing the old content — most of its value is in what you deliberately keep.
...
**Step 4 — Declare the Craft Read** (surface kind, audience, theme, `DESIGN_VARIANCE`, signature bet) per `../ui-craft/references/craft-intent.md`. Redesigns default to variance 4–5: the brand is already committed, so spend boldness on typography and composition, not on a new identity.

**Step 5 — Modernize with levers, in this order:** typography (scale + pairing per `../ui-craft/references/typography.md`) → spacing rhythm → color tokens (`../ui-craft/references/color.md`) → imagery treatment → motion (entrance/hover only where focal). Fix every detector finding from Step 1 along the way.
